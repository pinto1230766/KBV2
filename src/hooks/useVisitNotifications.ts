...
export function useVisitNotifications() {
  ...
  const scheduleVisitReminder = useCallback(async (visit: Visit) => {
    if (!settings.notifications.enabled) return;

    const visitDate = new Date(visit.visitDate);
    // Programmer les rappels pour chaque délai configuré
    const reminders = settings.notifications.reminderDays.length > 0 
      ? settings.notifications.reminderDays 
      : [3]; // Par défaut 3 jours avant
          ...
          notifications: [
            {
              ...
              id: parseInt(visit.id.replace(/\D/g, '').substring(0, 5)) + daysBefore * 10000 || Math.floor(Math.random() * 100000),
