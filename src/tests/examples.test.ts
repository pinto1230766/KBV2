  ...
  tests: [
    ...
    },
    {
      ...
      fn: async () => {
        ...
        const results = testValidation(mockSchema, mockSpeaker, {
          ...mockSpeaker,
          email: 'invalid-email',
        });
        const validResult = results[0];
      ...
      fn: async () => {
        const mockSchema = {
          parse: (data: any) => {
            if (
              !data.title ||
              !['scheduled', 'confirmed', 'completed', 'cancelled'].includes(data.status)
            ) {
        ...
        const results = testValidation(mockSchema, mockVisit, {
          ...mockVisit,
    ...
    },
    {
      ...
      fn: async () => {
        const mockSchema = {
          parse: (data: any) => {
            if (
              !data.subject ||
              !data.content ||
              !['draft', 'sent', 'delivered', 'read'].includes(data.status)
            ) {
        ...
        const results = testValidation(mockSchema, mockMessage, {
          ...mockMessage,
        ...
        const actions = [
          {
            name: 'setUser',
            action: () => {
              mockStore.getState().user = { id: '1', name: 'Test' } as any;
            },
...
export { validationTests, utilityTests, performanceTests, apiTests, accessibilityTests };
