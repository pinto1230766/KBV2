  ...
  updateActivity(): void {
    try {
      ...
      if (session) {
        ...
        localStorage.setItem(SECURITY_CONFIG.STORAGE_KEYS.SESSION, JSON.stringify(session));
      }
    } catch (error) {
...
class SecureTokenManager {
  ...
  async setTokens(tokens: AuthTokens, rememberMe: boolean = false): Promise<void> {
    try {
      ...
      storage.setItem(SECURITY_CONFIG.STORAGE_KEYS.ENCRYPTED_TOKENS, JSON.stringify(encryptedData));
      ...
      refreshToken: async () => {
        try {
          ...
          const rememberMe = Boolean(
            localStorage.getItem(SECURITY_CONFIG.STORAGE_KEYS.REMEMBER_ME)
          );
...
export { SecureTokenManager, SessionManager, SecureAuthApiClient };
