...
export const Planning: React.FC = () => {
  ...
  const [simpleMessageModal, setSimpleMessageModal] = useState<{
    isOpen: boolean;
    action: string;
    visit: Visit | null;
    speaker: any;
  ...
  return (
    <div className='space-y-6 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6'>
      ...
      {simpleMessageModal.isOpen && (
        <SimpleMessageModal
          isOpen={simpleMessageModal.isOpen}
          onClose={() => setSimpleMessageModal({ ...simpleMessageModal, isOpen: false })}
          visit={simpleMessageModal.visit || undefined}
          speaker={simpleMessageModal.speaker}
          host={simpleMessageModal.host}
          action={simpleMessageModal.action}
        />
      )}
    </div>
  );
