...
export function DataProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (shouldForceReload) {
        ...
        mergedData = {
          ...
          // Préserver les hôtes personnalisés mais supprimer les doublons (par nom, insensible à la casse)
          hosts: [...(saved?.hosts || []), ...completeData.hosts].filter(
            (host, index, arr) => arr.findIndex((h) => h.nom.toLowerCase() === host.nom.toLowerCase()) === index
          ),
          // Dédupliquer les orateurs par ID
          speakers: [...(completeData.speakers), ...(saved?.speakers || [])].filter(
            (speaker, index, arr) => arr.findIndex((s) => s.id === speaker.id) === index
          ),
          dataVersion: '1.4.0', // Mettre à jour la version
        };
      }
  ...
  const addHost = (host: Host) => {
    setData((d) => {
      // Vérifier si un hôte avec le même nom existe déjà (éviter les doublons)
      const exists = d.hosts.some((h) => h.nom.toLowerCase() === host.nom.toLowerCase());
      if (exists) {
        console.warn(`Host "${host.nom}" already exists, skipping duplicate`);
        return d;
      }
      return { ...d, hosts: [...d.hosts, host] };
    });
  };
