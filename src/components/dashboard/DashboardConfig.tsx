...
interface DashboardConfigState {
  ...
  widgets: WidgetConfig[];
  setWidgetVisibility: (_widgetId: string, _visible: boolean) => void;
  setWidgetOrder: (_widgetId: string, _order: number) => void;
  setWidgetSize: (_widgetId: string, _size: WidgetConfig['size']) => void;
  reorderWidgets: (_startIndex: number, _endIndex: number) => void;
  saveLayout: (_name: string) => void;
  loadLayout: (_layoutId: string) => void;
  resetToDefault: () => void;
}
    ...
    (set, get) => ({
      ...
      reorderWidgets: (_startIndex, _endIndex) => {
        set((state) => {
          ...
          return { widgets: result.map((w, i) => ({ ...w, order: i })) };
        });
      },
      saveLayout: (_name) => {
...
interface WidgetItemProps {
  widget: WidgetConfig;
  onToggle: () => void;
  onSizeChange: (_size: WidgetConfig['size']) => void;
  isDragging?: boolean;
}
