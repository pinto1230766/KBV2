    ...
    <div className={cn('space-y-4', className)}>
      ...
      >
        ...
        <p className='text-xs text-gray-500 dark:text-gray-500'>
          Formats acceptés: {allowedExtensions.join(', ')} • Taille max:{' '}
          ...
          <div className='space-y-2 max-h-60 overflow-y-auto'>
            {files.map((fileWithValidation, index) => (
              ...
              >
                ...
                {/* Icône de fichier */}
                {!fileWithValidation.preview && <File className='w-10 h-10 text-gray-400' />}
...
export function ImageUpload(props: Omit<FileUploadProps, 'accept'>) {
  return <FileUpload {...props} accept='image/*' showPreview={true} />;
}

// Composant spécialisé pour les documents
export function DocumentUpload(props: Omit<FileUploadProps, 'accept'>) {
  return <FileUpload {...props} accept='.pdf,.doc,.docx,.txt' showPreview={false} />;
}
