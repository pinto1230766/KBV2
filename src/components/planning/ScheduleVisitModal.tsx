                 ...
                 {[1, 2, 3].map(step => (
                   <div key={step} className="flex items-center">
                      <div className={cn(
                        ...
                        currentStep >= step ? "bg-white text-indigo-600 shadow-md scale-110" : "bg-indigo-900/40 text-indigo-300"
                      )}>
                         {step}
                      </div>
                      {step < 3 && <div className={cn("w-8 md:w-16 h-1 mx-2 rounded-full", currentStep > step ? "bg-white/50" : "bg-indigo-900/20")} />}
                   </div>
                 ))}
                 <span className="ml-auto text-xs font-bold uppercase tracking-widest opacity-80">
                    {currentStep === 1 ? "Date & Orateur" : currentStep === 2 ? "Logistique" : "Confirmation"}
                 </span>
              </div>
           </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8">
             ...
             <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                      ...
                      <div className="relative">
                         ...
                         <input 
                           ...
                           className={cn(
                             ...
                             dateError ? "focus:ring-red-500 bg-red-50 dark:bg-red-900/20" : "focus:ring-indigo-500"
                           )}
                         />
                      </div>
                      {dateError && <p className="text-xs text-red-600 font-medium pl-1">{dateError}</p>}
                   </div>
                   <div className="space-y-2">
                ...
                {dateConflict.has && (
                   <div className="p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-800 rounded-xl flex items-start gap-3">
                      ...
                      <div>
                         <h4 className="text-sm font-bold text-orange-800 dark:text-orange-200">Attention : Conflit potentiel</h4>
                ...
                {/* Speaker Selection */}
                <div className="space-y-2">
                   <label className="text-xs font-bold text-gray-500 uppercase tracking-widest pl-1">Orateur</label>
                   <div className="relative">
                ...
                <div className="space-y-4 pt-2 border-t border-gray-100 dark:border-gray-800">
                   ...
                   <div className="grid grid-cols-[1fr,2fr] gap-4">
                      ...
                      <datalist id="talks-list">
                         {publicTalks.map(t => <option key={t.number} value={t.number}>{t.theme}</option>)}
                      </datalist>
                   </div>
                </div>
             </div>
           ...
           {currentStep === 2 && (
             <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                <div className="space-y-3">
                   <label className="text-xs font-bold text-gray-500 uppercase tracking-widest pl-1">Mode de diffusion</label>
                ...
                {formData.locationType === 'physical' && !isLocalSpeaker && (
                   <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-gray-800 animate-in fade-in duration-300">
                      ...
                      <div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl flex gap-3 text-blue-700 dark:text-blue-300 text-xs leading-relaxed">
                         ...
                         <p>L'orateur vient d'une autre congrégation. Pensez à vérifier ses besoins en matière de repas ou d'hébergement.</p>
                      </div>
                   </div>
                )}
           ...
           {currentStep === 3 && (
             <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300 text-center">
                ...
                <h3 className="text-xl font-black text-gray-900 dark:text-white">Tout semble correct ?</h3>
                ...
                <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-6 text-left space-y-4">
                   <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-2">
                      ...
                      <span className="text-sm font-bold text-gray-900 dark:text-white">
                         {new Date(formData.visitDate!).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}
                      </span>
                   </div>
                   <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-2">
                       <span className="text-xs font-bold text-gray-400 uppercase">Orateur</span>
                       <span className="text-sm font-bold text-gray-900 dark:text-white">{formData.nom}</span>
                   </div>
                   <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-2">
                       ...
                       <span className="text-sm font-medium text-gray-900 dark:text-white text-right max-w-[200px] truncate">
                          {formData.talkNoOrType ? `N°${formData.talkNoOrType} - ${formData.talkTheme}` : 'Non défini'}
                       </span>
                   </div>
