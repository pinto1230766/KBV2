...
export const PlanningWeekView: React.FC<PlanningWeekViewProps> = ({ visits, onVisitClick }) => {
  ...
  const visitsByDay = useMemo(() => {
    ...
    weekDays.forEach(day => {
      ...
      grouped[dayKey] = visits.filter(visit => 
        isSameDay(new Date(visit.visitDate), day)
      ...
      <div className="grid grid-cols-7 divide-x divide-gray-200 dark:divide-gray-700">
        {weekDays.map((day) => {
          ...
          return (
            ...
            >
              {/* En-tête du jour */}
              <div className={`p-3 border-b border-gray-200 dark:border-gray-700 ${today ? 'bg-primary-100 dark:bg-primary-900/30' : 'bg-gray-50 dark:bg-gray-900/50'}`}>
                <div className="text-center">
                  <div className={`text-xs font-medium uppercase ${today ? 'text-primary-700 dark:text-primary-300' : 'text-gray-500 dark:text-gray-400'}`}>
                    {format(day, 'EEE', { locale: fr })}
                  </div>
                  <div className={`text-xl font-bold mt-1 ${today ? 'text-primary-900 dark:text-primary-100' : 'text-gray-900 dark:text-white'}`}>
                    {format(day, 'd')}
                  </div>
                  {dayVisits.length > 0 && (
