      ...
      >
        <div className="flex flex-col h-[85vh] overflow-hidden">
               ...
               <div className="relative z-10 px-8 py-8">
                   <div className="flex justify-between items-start mb-6">
                       <div className="inline-flex items-center px-3 py-1 bg-white/10 rounded-full backdrop-blur-md border border-white/10">
                           ...
                           <span className="text-[10px] font-bold uppercase tracking-widest text-white/90">Évaluation de visite</span>
                       </div>
                   ...
                   <div className="space-y-1">
                       ...
                       <div className="flex items-center gap-3 text-blue-200/80 text-sm font-medium">
                           <span>{new Date(visit.visitDate).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}</span>
                   ...
                   {visit.talkTheme && (
                     <div className="mt-4 p-3 bg-white/5 border border-white/5 rounded-xl backdrop-blur-sm inline-block">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-white/50 mb-1">Discours</div>
                        <div className="text-sm font-medium text-white/90 flex items-center gap-2">
                           <span className="px-1.5 py-0.5 bg-blue-500/20 text-blue-300 rounded text-xs font-bold">{visit.talkNoOrType}</span>
                           {visit.talkTheme}
                        </div>
                     </div>
                   )}
           ...
           <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar bg-slate-50 dark:bg-slate-900/50">
               ...
               <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 space-y-6">
                   <div className="flex items-center gap-4 border-b border-slate-100 dark:border-slate-700 pb-4">
                       <div className="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-2xl shadow-sm">🎤</div>
                       <div>
                           <h3 className="text-lg font-bold text-slate-900 dark:text-white">Performance de l'orateur</h3>
                           <p className="text-xs text-slate-500 dark:text-slate-400">Qualité de l'enseignement & rhétorique</p>
                       </div>
                       {speakerRating > 0 && (
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                       {FEEDBACK_CATEGORIES.map((category) => (
                           <div key={category.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50 hover:bg-white dark:hover:bg-slate-700 border border-transparent hover:border-slate-200 dark:hover:border-slate-600 transition-all">
                       ...
                       <div className="flex justify-center">
                            <div className="flex gap-2">
                                {[1, 2, 3, 4, 5].map((star) => (
                                  ...
                                  >
                                    <Star className={`w-8 h-8 ${star <= hostRating ? 'fill-green-400 text-green-400' : 'text-slate-200 dark:text-slate-600'}`} />
                                  </button>
                                ))}
                            </div>
                       </div>
                            ...
                            <div className="flex gap-2">
                                {[1, 2, 3, 4, 5].map((star) => (
                                  <button
                                    ...
                                    className="transition-transform hover:scale-110"
                                  >
                                    <Star className={`w-8 h-8 ${star <= organizationRating ? 'fill-purple-400 text-purple-400' : 'text-slate-200 dark:text-slate-600'}`} />
                                  </button>
                                ))}
                            </div>
                       </div>
               ...
               <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-700">
                   <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-4">Pistes d'amélioration</h3>
               ...
               <div className="flex items-center gap-3 px-2">
                   ...
                   >
                       <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${isPrivate ? 'translate-x-6' : ''}`} />
                   </div>
                   <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Marquer comme confidentiel</span>
               </div>
           </div>
           ...
           <div className="p-4 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700 flex justify-between items-center shrink-0">
               <button onClick={onClose} className="px-6 py-3 text-sm font-bold text-slate-500 uppercase tracking-widest hover:text-slate-800 dark:hover:text-white transition-colors">
                   Annuler
               </button>
               <Button 
