    ...
    <Card>
      ...
      <CardBody>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-center">
          {data.map((item) => (
            <div key={item.name} className="flex flex-col items-center p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50">
              <span className="text-xs text-gray-500 mb-1">{item.name}</span>
              <span className="text-lg font-bold text-gray-900 dark:text-white">{item.score.toFixed(1)}</span>
            </div>
          ))}
        </div>
      </CardBody>
