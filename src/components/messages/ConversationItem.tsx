    ...
    >
      ...
      <div className='flex-1 min-w-0'>
        <div className='flex justify-between items-baseline mb-1'>
          <h3
            className={`font-semibold truncate ${isActive ? 'text-primary-900 dark:text-primary-100' : 'text-gray-900 dark:text-white'}`}
          >
            {speaker.nom}
          </h3>
          {lastVisit && (
        ...
        <p className='text-sm text-gray-500 dark:text-gray-400 truncate'>{speaker.congregation}</p>
        {lastVisit && (
          <p className='text-xs text-gray-400 mt-1 truncate'>
            {lastVisit.talkNoOrType ? (
              <>
                N°{lastVisit.talkNoOrType}
                {(lastVisit.talkTheme || getTalkTitle(lastVisit.talkNoOrType)) &&
                  ` - ${lastVisit.talkTheme || getTalkTitle(lastVisit.talkNoOrType)}`}
              </>
            ) : (
              'Visite prévue'
