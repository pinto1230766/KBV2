...
import { r as h } from './charts-CCOFnOOy.js';
import { S as O } from './Select-Cs8xHSai.js';
import { a as L } from './copy-DwuKvwjM.js';
const ae = x('Check', [['path', { d: 'M20 6 9 17l-5-5', key: '1gmf2c' }]]);
const _ = x('Send', [
...
function V(n, t, s) {
  ...
  return (
    t === 'female' &&
      ((e = e.replace(/Bonjour Frère/g, 'Bonjour Sœur')),
      (e = e.replace(/Frère \*{speakerName}\*/g, 'Sœur *{speakerName}*')),
      (e = e.replace(/notre orateur invité/g, 'notre oratrice invitée')),
      (e = e.replace(/Olá Irmão/g, 'Olá Irmã')),
      (e = e.replace(/Irmão \*{speakerName}\*/g, 'Irmã *{speakerName}*')),
      (e = e.replace(/nosso orador convidado/g, 'nossa oradora convidada')),
      (e = e.replace(/Bon dia Mon/g, 'Bon dia Mana')),
      (e = e.replace(/Mon \*{speakerName}\*/g, 'Mana *{speakerName}*'))),
    s === 'female' &&
      ((e = e.replace(/Frère \*{hostName}\*/g, 'Sœur *{hostName}*')),
      (e = e.replace(/notre frère/g, 'notre sœur')),
      (e = e.replace(/Irmão \*{hostName}\*/g, 'Irmã *{hostName}*')),
      (e = e.replace(/nosso irmão/g, 'nossa irmã')),
      (e = e.replace(/Mon \*{hostName}\*/g, 'Mana *{hostName}*')),
      (e = e.replace(/nu mon {hostName}/g, 'nu mana {hostName}'))),
    s === 'couple' &&
      ((e = e.replace(/Frère \*{hostName}\*/g, '*{hostName}*')),
      (e = e.replace(/tu vas bien/g, 'vous allez bien')),
      (e = e.replace(/Irmão \*{hostName}\*/g, '*{hostName}*')),
      (e = e.replace(/estejas bem/g, 'estejam bem')),
      (e = e.replace(/tua disponibilidade/g, 'vossa disponibilidade')),
      (e = e.replace(/teu acolhimento/g, 'vosso acolhimento')),
      (e = e.replace(/Mon \*{hostName}\*/g, '*{hostName}*')),
      (e = e.replace(/bu sta bon/g, 'nhós sta bon'))),
    e
  );
}
function K(n, t, s, e, a, m, o, p) {
  const c = J[o]?.[a]?.[m];
  if (!c) return `Modèle non trouvé pour: ${o}/${a}/${m}`;
  let l = W(c, n, s, e);
  return ((l = V(l, t?.gender, s?.gender)), l);
}
function W(n, t, s, e) {
  let a = n;
  ((a = a.replace(/{speakerName}/g, t.nom)),
    (a = a.replace(/{congregation}/g, t.congregation)),
    (a = a.replace(/{speakerPhone}/g, t.telephone || '(non renseigné)')));
  const m = t.host !== 'À définir' ? t.host : '(non assigné)';
  if (
    ((a = a.replace(/{hostName}/g, m)),
    (a = a.replace(/{hostPhone}/g, s?.telephone || '(non renseigné)')),
    (a = a.replace(/{hostAddress}/g, s?.address || '(non renseignée)')),
    (a = a.replace(/{visitDate}/g, F(t.visitDate))),
    (a = a.replace(/{visitTime}/g, t.visitTime)),
    t.talkNoOrType &&
      t.talkTheme &&
      ((a = a.replace(/{talkNo}/g, String(t.talkNoOrType))),
      (a = a.replace(/{talkTheme}/g, t.talkTheme))),
    (a = a.replace(/{hospitalityOverseer}/g, e.hospitalityOverseer)),
    (a = a.replace(/{hospitalityOverseerPhone}/g, e.hospitalityOverseerPhone)),
    (a = a.replace(/{congregationName}/g, e.name)),
    !t.communicationStatus || Object.keys(t.communicationStatus).length === 0)
  ) {
    const p = H(e.name);
    a = a.replace(/{firstTimeIntroduction}/g, p);
  } else a = a.replace(/{firstTimeIntroduction}/g, '');
  return a;
}
function H(n) {
  return `

Je suis le responsable de l'accueil pour le ${n}.`;
}
function re(n, t, s, e) {
  const a = U[s];
  if (!a) return `Modèle non trouvé pour la langue: ${s}`;
  const m = n.map((p, c) => {
    const l = F(p.visitDate, s);
    return `${c + 1}. *${p.nom}* (${p.congregation}) - ${l} à ${p.visitTime}`;
  }).join(`
`);
  let o = a;
  return (
    (o = o.replace(/{visitsList}/g, m)),
    (o = o.replace(/{hospitalityOverseer}/g, t.hospitalityOverseer)),
    (o = o.replace(/{hospitalityOverseerPhone}/g, t.hospitalityOverseerPhone)),
    o
  );
}
async function Q(n, t, s, e = 0.7) {
  try {
    const { GoogleGenerativeAI: a } = await S(async () => {
        const { GoogleGenerativeAI: g } = await import('./index-ySGxHPpP.js');
        return { GoogleGenerativeAI: g };
      }, []),
      o = new a(s).getGenerativeModel({
        model: 'gemini-pro',
        generationConfig: { temperature: e, maxOutputTokens: 1e3 },
      }),
      c = `${{ rewrite: 'Réécris ce message de manière plus claire et professionnelle tout en gardant le sens:', shorten: 'Raccourcis ce message en gardant uniquement les informations essentielles:', expand: 'Développe ce message en ajoutant plus de détails et de chaleur:', formal: 'Réécris ce message avec un ton plus formel et respectueux:', friendly: 'Réécris ce message avec un ton plus amical et chaleureux:' }[t]}

${n}`;
    return (await (await o.generateContent(c)).response).text();
  } catch (a) {
    throw (
      console.error('Error generating with AI:', a),
      new Error("Erreur lors de la génération avec l'IA")
    );
  }
}
const te = ({
  ...
  speaker: s,
  visit: e,
  initialChannel: a = 'whatsapp',
  initialType: m = 'reminder-7',
}) => {
  const { settings: o } = z(),
    { hosts: p, congregationProfile: c } = $(),
    { addToast: l } = E(),
    [u, g] = h.useState(''),
    [v, I] = h.useState(a),
    [f, T] = h.useState(m),
    [N, C] = h.useState(o.language),
    [P, y] = h.useState(!1),
    [j, M] = h.useState(!1),
    b = async (i) => {
      if (!o.aiSettings.apiKey) {
        l('Veuillez configurer la clé API dans les paramètres', 'error');
        return;
      }
      (y(!0), M(!1));
      try {
        const d = await Q(u, i, o.aiSettings.apiKey, o.aiSettings.temperature);
        (g(d), l("Message amélioré par l'IA", 'success'));
      } catch (d) {
        (console.error(d), l('Erreur lors de la génération IA', 'error'));
      } finally {
        y(!1);
      }
    };
  ...
  const w = async () => {
      y(!0);
      try {
        const i = p.find((D) => D.nom === e.host),
          d = K(e, s, i, c, f, 'speaker', N);
        g(d);
      } catch {
        l('Erreur lors de la génération du message', 'error');
      } finally {
        y(!1);
      }
    },
    ...
    B = () => {
      if (v === 'whatsapp') {
        const i = encodeURIComponent(u),
          d = s.telephone?.replace(/\D/g, '') || '';
        window.open(`https://wa.me/${d}?text=${i}`, '_blank');
      } else if (v === 'email') {
        const i = encodeURIComponent(`Visite à KBV Lyon - ${e.talkTheme}`),
          d = encodeURIComponent(u);
        window.open(`mailto:${s.email}?subject=${i}&body=${d}`, '_blank');
      } else {
        const i = encodeURIComponent(u);
        window.open(`sms:${s.telephone}?body=${i}`, '_blank');
      }
      t();
    };
  return r.jsx(R, {
    isOpen: n,
    onClose: t,
    title: `Message pour ${s.nom}`,
    size: 'lg',
    footer: r.jsxs(r.Fragment, {
      children: [
        r.jsx(k, { variant: 'ghost', onClick: t, children: 'Annuler' }),
        r.jsx(k, {
          variant: 'secondary',
          onClick: A,
          leftIcon: r.jsx(L, { className: 'w-4 h-4' }),
          children: 'Copier',
        }),
        r.jsxs(k, {
          onClick: B,
          leftIcon: r.jsx(_, { className: 'w-4 h-4' }),
          children: [
            'Envoyer (',
            v === 'whatsapp' ? 'WhatsApp' : v === 'email' ? 'Email' : 'SMS',
            ')',
          ],
        }),
      ],
    ...
    children: r.jsxs('div', {
      ...
      children: [
        r.jsxs('div', {
          className:
            'flex flex-wrap gap-4 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700',
          children: [
            r.jsx('div', {
            ...
            r.jsxs('div', {
              ...
              children: [
                r.jsx(k, {
                  ...
                  onClick: w,
                  isLoading: P,
                  leftIcon: r.jsx(q, { className: 'w-4 h-4' }),
                  children: 'Régénérer',
                }),
                o.aiSettings.enabled &&
                  r.jsxs('div', {
                    className: 'relative',
                    children: [
                      r.jsx(k, {
                        ...
                        leftIcon: r.jsx(G, { className: 'w-4 h-4' }),
                        onClick: () => M(!j),
                        children: 'Améliorer avec IA',
                      }),
                      j &&
                        r.jsxs('div', {
                          className:
                            'absolute bottom-full right-0 mb-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-50',
                          children: [
                            r.jsx('button', {
                              onClick: () => b('rewrite'),
                              className:
                                'w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm',
                              children: 'Réécrire',
                            }),
                            r.jsx('button', {
                              onClick: () => b('shorten'),
                            ...
                            r.jsx('button', {
                              ...
                              className:
                                'w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm',
                              children: 'Plus formel',
                            }),
                            r.jsx('button', {
                              onClick: () => b('friendly'),
                              className:
                                'w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm',
                              children: 'Plus amical',
                            }),
                          ],
                        }),
                    ],
...
export { ae as C, te as M, _ as S, re as g };
//# sourceMappingURL=MessageGeneratorModal-DsejHfJr.js.map
