...
const K = 'https://generativelanguage.googleapis.com',
  U = 'v1',
  F = '0.1.3',
  B = 'genai-js';
var E;
...
class g {
  constructor(n, t, o, s) {
    ((this.model = n), (this.task = t), (this.apiKey = o), (this.stream = s));
  }
  toString() {
    let n = `${K}/${U}/models/${this.model}:${this.task}`;
    return (this.stream && (n += '?alt=sse'), n);
  }
}
function Y() {
  return `${B}/${F}`;
...
async function R(e, n) {
  ...
  try {
    if (
      ...
      throw new Error(`[${t.status} ${t.statusText}] ${o}`);
    }
  } catch (o) {
    const s = new h(`Error fetching from ${e.toString()}: ${o.message}`);
    ...
    (e.text = () => {
      if (e.candidates && e.candidates.length > 0) {
        if (
          ...
          throw new M(`${p(e)}`, e);
        return x(e);
      } else if (e.promptFeedback) throw new M(`Text not available. ${p(e)}`, e);
      return '';
    }),
    e
  );
...
function x(e) {
  ...
  return !(
    (s =
      (o =
        (t = (n = e.candidates) === null || n === void 0 ? void 0 : n[0].content) === null ||
        t === void 0
          ? void 0
          : t.parts) === null || o === void 0
        ? void 0
        : o[0]) === null || s === void 0
  ) && s.text
...
const $ = [S.RECITATION, S.SAFETY];
function G(e) {
  return !!e.finishReason && $.includes(e.finishReason);
}
function p(e) {
  var n, t, o;
  let s = '';
  if ((!e.candidates || e.candidates.length === 0) && e.promptFeedback)
    ((s += 'Response was blocked'),
      !((n = e.promptFeedback) === null || n === void 0) &&
        n.blockReason &&
        (s += ` due to ${e.promptFeedback.blockReason}`),
      !((t = e.promptFeedback) === null || t === void 0) &&
        t.blockReasonMessage &&
        (s += `: ${e.promptFeedback.blockReasonMessage}`));
  else if (!((o = e.candidates) === null || o === void 0) && o[0]) {
    const i = e.candidates[0];
    G(i) &&
      ((s += `Candidate was blocked due to ${i.finishReason}`),
...
function j(e, n, t) {
  ...
  function a(d) {
    d.value instanceof _ ? Promise.resolve(d.value.v).then(u, m) : I(i[0][2], d);
  }
  function u(d) {
    r('next', d);
  }
  ...
  function I(d, f) {
    (d(f), i.shift(), i.length && r(i[0][0], i[0][1]));
  }
}
const L = /^data\: (.*)(?:\n\n|\r\r|\r\n\r\n)/;
function k(e) {
  const n = e.body.pipeThrough(new TextDecoderStream('utf8', { fatal: !0 })),
    t = q(n),
    [o, s] = t.tee();
  return { stream: V(o), response: J(s) };
}
async function J(e) {
  const n = [],
    t = e.getReader();
...
function V(e) {
  return j(this, arguments, function* () {
    const t = e.getReader();
    for (;;) {
      const { value: o, done: s } = yield _(t.read());
      if (s) break;
      yield yield _(A(o));
    }
  });
...
function q(e) {
  ...
  return new ReadableStream({
    start(o) {
      ...
      return i();
      function i() {
        return n.read().then(({ value: c, done: r }) => {
          if (r) {
            if (s.trim()) {
              o.error(new h('Failed to parse stream'));
              return;
            }
            o.close();
            return;
          ...
          for (; a; ) {
            try {
              ...
              o.error(new h(`Error parsing JSON response: "${a[1]}"`));
              return;
            }
            (o.enqueue(u), (s = s.substring(a[0].length)), (a = s.match(L)));
          }
          return i();
        });
      }
...
function W(e) {
  const n = e[e.length - 1],
    t = { promptFeedback: n?.promptFeedback };
  for (const o of e)
    if (o.candidates)
      for (const s of o.candidates) {
        ...
        if (
          ...
          t.candidates[i] || (t.candidates[i] = { index: s.index }),
          ...
          t.candidates[i].content ||
            (t.candidates[i].content = { role: s.content.role || 'user', parts: [{ text: '' }] });
          for (const c of s.content.parts)
            c.text && (t.candidates[i].content.parts[0].text += c.text);
        }
      }
  return t;
}
async function P(e, n, t) {
  const o = new g(n, E.STREAM_GENERATE_CONTENT, e, !0),
    s = await R(o, JSON.stringify(t));
  return k(s);
}
async function H(e, n, t) {
  const o = new g(n, E.GENERATE_CONTENT, e, !1),
    i = await (await R(o, JSON.stringify(t))).json();
  return { response: A(i) };
}
function l(e, n) {
  let t = [];
  if (typeof e == 'string') t = [{ text: e }];
  else for (const o of e) typeof o == 'string' ? t.push({ text: o }) : t.push(o);
  return { role: n, parts: t };
}
function O(e) {
  return e.contents ? e : { contents: [l(e, 'user')] };
}
function X(e) {
  return typeof e == 'string' || Array.isArray(e) ? { content: l(e, 'user') } : e;
}
...
class Q {
  constructor(n, t, o) {
    ((this.model = t),
      ...
      (this._apiKey = n),
      o?.history &&
        (this._history = o.history.map((s) => {
          if (!s.role) throw new Error('Missing role for history item: ' + JSON.stringify(s));
          return l(s.parts, s.role);
        })));
  }
  async getHistory() {
  ...
  async sendMessage(n) {
    ...
    return (
      (this._sendPromise = this._sendPromise
        .then(() => H(this._apiKey, this.model, i))
        .then((r) => {
          var a;
          if (r.response.candidates && r.response.candidates.length > 0) {
            this._history.push(s);
            const u = Object.assign(
              { parts: [], role: 'model' },
              (a = r.response.candidates) === null || a === void 0 ? void 0 : a[0].content
            );
            this._history.push(u);
          } else {
            const u = p(r.response);
            u &&
              console.warn(
                `sendMessage() was unsuccessful. ${u}. Inspect response object for details.`
              );
          }
          c = r;
        })),
      ...
      (this._sendPromise = this._sendPromise
        ...
        .then((r) => {
          if (r.candidates && r.candidates.length > 0) {
            this._history.push(s);
            const a = Object.assign({}, r.candidates[0].content);
            (a.role || (a.role = 'model'), this._history.push(a));
          } else {
            const a = p(r);
            a &&
              console.warn(
                `sendMessageStream() was unsuccessful. ${a}. Inspect response object for details.`
              );
          }
        })
        .catch((r) => {
          r.message !== b && console.error(r);
        })),
      c
    );
  }
...
async function z(e, n, t) {
  const o = new g(n, E.COUNT_TOKENS, e, !1);
  return (await R(o, JSON.stringify(Object.assign(Object.assign({}, t), { model: n })))).json();
}
async function Z(e, n, t) {
  const o = new g(n, E.EMBED_CONTENT, e, !1);
  return (await R(o, JSON.stringify(t))).json();
}
async function ee(e, n, t) {
  const o = new g(n, E.BATCH_EMBED_CONTENTS, e, !1),
    s = t.requests.map((c) => Object.assign(Object.assign({}, c), { model: `models/${n}` }));
  return (await R(o, JSON.stringify({ requests: s }))).json();
}
class te {
  constructor(n, t) {
    var o;
    ((this.apiKey = n),
      t.model.startsWith('models/')
        ? (this.model = (o = t.model.split('models/')) === null || o === void 0 ? void 0 : o[1])
        : (this.model = t.model),
  ...
  async generateContent(n) {
    ...
    return H(
      ...
      this.model,
      Object.assign(
        { generationConfig: this.generationConfig, safetySettings: this.safetySettings },
        t
      )
    );
  }
  ...
  startChat(n) {
    return new Q(this.apiKey, this.model, n);
  }
  async countTokens(n) {
    const t = O(n);
    return z(this.apiKey, this.model, t);
  }
  ...
  async batchEmbedContents(n) {
    return ee(this.apiKey, this.model, n);
  }
}
class ne {
  constructor(n) {
...
export {
  ...
  Q as ChatSession,
  S as FinishReason,
  te as GenerativeModel,
  ne as GoogleGenerativeAI,
  N as HarmBlockThreshold,
  T as HarmCategory,
  y as HarmProbability,
  w as TaskType,
};
//# sourceMappingURL=index-ySGxHPpP.js.map
